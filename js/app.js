...
function renderYardimCategories() {
  ...
  YARDIM_CATEGORIES.forEach(cat => {
    ...
    btn.addEventListener('click', () => {
      ...
      cityField.style.display = 'block';
      showResultsBtn.style.display = 'block';
      showResultsBtn.textContent = '📍 Konumumu Paylaş ve En Yakına Ulaş';
      resultsSection.style.display = 'none';
...
function renderFirmResults(firms, baseTitle, city, emptyText, waPurpose, coords) {
  resultsList.innerHTML = "";
...
showResultsBtn.addEventListener('click', () => {
  ...
  if (currentSubcat.isYardim) {
    const firms = YARDIM_FIRMS.filter(f => f.categories.includes(currentSubcat.key) && f.city === city);
    ...
    if (!navigator.geolocation || firms.length === 0) {
      renderFirmResults(firms.sort((a,b) => a.name.localeCompare(b.name, 'tr')), baseTitle, city, emptyText, waPurpose, null);
      return;
    }
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        const coords = { lat: pos.coords.latitude, lng: pos.coords.longitude };
        const sorted = firms.slice().sort((a, b) => distanceKm(coords, a) - distanceKm(coords, b));
        renderFirmResults(sorted, baseTitle, city, emptyText, waPurpose, coords);
      },
      () => {
        renderFirmResults(firms.sort((a,b) => a.name.localeCompare(b.name, 'tr')), baseTitle, city, emptyText, waPurpose, null);
      }
