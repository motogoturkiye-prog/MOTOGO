  ...
  try {
    ...
    if (firmalar) {
      ...
      firmalar.forEach(r => {
        ...
        grouped[key].push({
          ...
          website: r.website, description: r.aciklama, photo: r.foto_url, siraNo: r.sira_no
        });
      });
      BRAND_FIRMS = grouped;
    }
    ...
    if (kiralik) {
      KIRALIK_FIRMS = kiralik.map(r => ({
        ...
        phone: r.telefon, instagram: r.instagram, description: r.aciklama, photo: r.foto_url, siraNo: r.sira_no
      }));
    }
    ...
    if (avantaj) {
      AVANTAJ_FIRMS = avantaj.map(r => ({
        ...
        whatsapp: r.whatsapp, phone: r.telefon, photo: r.foto_url, siraNo: r.sira_no
      }));
    }
    ...
    if (yardim) {
      YARDIM_FIRMS = yardim.map(r => ({
        ...
        categories: r.kategoriler || [], website: r.website, instagram: r.instagram, photo: r.foto_url, siraNo: r.sira_no
      }));
    }
...
function renderAvantajResults(firms, city) {
  ...
  firms.forEach(firm => {
    ...
    const bodyHtml = `
      ...
      ${firm.whatsapp ? `<a class="firm-whatsapp" style="display:inline-block;" href="https://wa.me/${firm.whatsapp}?text=${waMsg}" target="_blank">WhatsApp'tan Ulaş</a>` : ''}
    `;

    if (firm.photo) {
      card.className = 'firm-card firm-card-photo';
      card.innerHTML = `
        <img class="firm-card-photo-img photo-click" src="${firm.photo}" alt="${firm.name}">
        <div class="firm-card-photo-row">
          <div class="firm-card-photo-info">${bodyHtml}</div>
        </div>
      `;
    } else {
    ...
    const photoImg = card.querySelector('.photo-click');
    if (photoImg) {
      photoImg.addEventListener('click', () => openPhotoLightbox(firm.photo));
    }
...
function renderFirmResults(firms, baseTitle, city, emptyText, waPurpose, coords, showLocationBtn) {
  ...
  if (firms.length === 0) {
    ...
    firms.forEach(firm => {
      ...
      if (firm.photo) {
        card.classList.add('firm-card-photo');
        card.innerHTML = `
          <img class="firm-card-photo-img photo-click" src="${firm.photo}" alt="${firm.name}">
          <div class="firm-card-photo-row">
            <div class="firm-card-photo-info">
              <p class="firm-name" style="margin:0;">${firm.name}${subtitle ? ' - ' + subtitle : ''}</p>
              <p class="firm-meta">${distText}${firm.city || ""}</p>
              ${firm.description ? `<p class="firm-desc">${firm.description}</p>` : ''}
              <div class="firm-social-icons" style="margin-top:8px;">${socialLinks}</div>
            </div>
            <div class="firm-card-photo-btns">
      ...
      const inceleBtn = card.querySelector('.incele-btn');
      if (inceleBtn) {
        inceleBtn.addEventListener('click', (e) => {
