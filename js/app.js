...
// Varsayılan çalışma saati: Pazartesi-Cumartesi 09:00-19:00, Pazar kapalı.
// Bir firmanın saati farklıysa js/firms-data.js içinde o firmaya
// "hours" alanı eklenip bu varsayılan geçersiz kılınabilir.
const DEFAULT_HOURS = [null, [9,19], [9,19], [9,19], [9,19], [9,19], [9,19]]; // 0=Paz ... 6=Cmt

function isOpenNow(f){
  const hours = f.hours || DEFAULT_HOURS;
  const now = new Date();
  const today = hours[now.getDay()];
  if (!today) return false;
  const t = now.getHours() + now.getMinutes() / 60;
  return t >= today[0] && t < today[1];
}

function statusLabel(f){
  return isOpenNow(f) ? 'Açık' : 'Kapalı';
}
    ...
    row.innerHTML = `
      <div class="firm-info">
        <div>
          ...
          <div class="firm-meta">
            <span>${f.region || ''}</span>
            <span class="badge ${isOpenNow(f) ? 'acik' : 'kapali'}">${dist ? dist + ' · ' : ''}${statusLabel(f)}</span>
          </div>
        </div>
      </div>
