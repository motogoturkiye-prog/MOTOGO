  ...
  try {
    ...
    if (firmalar) {
      ...
      firmalar.forEach(r => {
        ...
        grouped[key].push({
          ...
          website: r.website, description: r.aciklama, photo: r.foto_url
        });
      });
      BRAND_FIRMS = grouped;
    }
    ...
    if (kiralik) {
      KIRALIK_FIRMS = kiralik.map(r => ({
        ...
        phone: r.telefon, instagram: r.instagram, description: r.aciklama, photo: r.foto_url
      }));
    }

    const { data: ekspertiz } = await sb.from('ekspertiz_firmalari').select('*');
    if (ekspertiz) {
    ...
    if (avantaj) {
      AVANTAJ_FIRMS = avantaj.map(r => ({
        ...
        whatsapp: r.whatsapp, phone: r.telefon, photo: r.foto_url
      }));
    }
    ...
    if (yardim) {
      YARDIM_FIRMS = yardim.map(r => ({
        ...
        categories: r.kategoriler || [], website: r.website, instagram: r.instagram, photo: r.foto_url
      }));
    }
    ...
    const { data: rotalar } = await sb.from('rotalar').select('*');
    if (rotalar) {
      ROTALAR = rotalar.map(r => ({ ad: r.ad, bolge: r.bolge, not_: r.not_metni, q: r.arama_terimi }));
    }

    const { data: ipuclari } = await sb.from('guvenlik_ipuclari').select('*');
    if (ipuclari) {
      GUVENLIK_IPUCLARI = ipuclari.map(r => r.ipucu);
    }

    const { data: etkinlikler } = await sb.from('etkinlikler').select('*');
    if (etkinlikler) {
    ...
    card.innerHTML = `
      <div class="avantaj-card-head">
        <div style="display:flex;align-items:center;gap:8px;">
          ${firm.photo ? `<img class="firm-thumb" src="${firm.photo}" alt="">` : ''}
          <div>
  ...
  if (firms.length === 0) {
    ...
    firms.forEach(firm => {
      ...
      if (firm.photo) {
        ...
        card.innerHTML = `
          <img class="firm-card-photo-img" src="${firm.photo}" alt="${firm.name}">
          <div class="firm-card-photo-body">
            <div>
            ...
            <div class="firm-card-photo-actions">
