...
const yardimGrid = document.getElementById('yardimGrid');
...
// ---------------------------------------------------------------------------
// Kategori satırı — Servis / Bayi / Yedek Parça / Aksesuar / Çekici / Lastik /
// Akü-Marş / Genel Arıza — hepsi tek satırda, aynı anda, hiçbiri saklı değil
// ---------------------------------------------------------------------------
renderSubcatGrid();
function renderSubcatGrid() {
  subcatGrid.innerHTML = "";
  yardimGrid.innerHTML = "";
  ...
  YARDIM_CATEGORIES.forEach(cat => {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'subcat-chip subcat-chip-yardim' + (yardimActive && selectedYardimCat === cat.id ? ' subcat-chip-active' : '');
...
brandInput.addEventListener('input', () => {
  ...
  blinkCursor.classList.remove('show');
...
function selectBrand(brand) {
  ...
  blinkCursor.style.left = measureTextWidth(brand.name, brandInput) + 'px';
  brandSuggestions.innerHTML = "";
  brandSuggestions.style.display = 'none';
