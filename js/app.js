...
showResultsBtn.addEventListener('click', () => {
  ...
  if (currentSubcat.isYardim) {
    ...
    const sourceFirms = currentSubcat.key === 'avantaj' ? AVANTAJ_FIRMS : KIRALIK_FIRMS;
    const firms = sourceFirms
      .filter(f => f.city === city)
      .sort((a, b) => a.name.localeCompare(b.name, 'tr'));
    const baseTitle = currentSubcat.label;
    const emptyText = currentSubcat.key === 'avantaj'
      ? "Bu şehirde henüz üye avantajlı firma yok — yakında eklenecek."
